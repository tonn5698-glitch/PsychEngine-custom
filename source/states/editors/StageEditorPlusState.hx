...
class StageEditorPlusState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	final minZoom = 0.1;
	final maxZoom = 2;
	...
	var stageSprites:Array<StageEditorPlusMetaSprite> = [];
	...
	var animationEditor:StageEditorPlusAnimationSubstate;
	...
	override function create()
	{
		...
		// StageEditor++: background grid
		gridBackdrop = new FlxBackdrop(FlxGridOverlay.createGrid(gridSize, gridSize, FlxG.width * 2, FlxG.height * 2, true, 0x66444444, 0x66CCCCCC));
		gridBackdrop.scrollFactor.set(1, 1);
		gridBackdrop.alpha = 0.4;
		gridBackdrop.visible = showGrid;
		gridBackdrop.cameras = [camGame];
	...
	function updateSpriteList()
	{
		...
		for (character in ['gf', 'dad', 'boyfriend'])
			if(!list.exists(character))
				stageSprites.push(new StageEditorPlusMetaSprite({type: character}, Reflect.field(this, character)));
	...
	function addSpriteListBox()
	{
		...
		var buttonDuplicate:PsychUIButton = new PsychUIButton(buttonX, buttonY + 90, 'Duplicate', function()
		{
			...
			var copiedMeta:StageEditorPlusMetaSprite = new StageEditorPlusMetaSprite(null, copiedSpr);
	...
	function spriteCreatePopup()
	{
		...
		var btn:PsychUIButton = new PsychUIButton(0, btnY, 'Solid Color', function() {
			var meta:StageEditorPlusMetaSprite = new StageEditorPlusMetaSprite({type: 'square', scale: [200, 200], name: findUnoccupiedName()}, new ModchartSprite());
	...
	function addDataTab()
	{
		...
		directoryDropDown = new PsychUIDropDownMenu(objX, objY, folderList, function(sel:Int, selected:String) {
			...
			MusicBeatState.switchState(new StageEditorPlusState(lastLoadedStage, stageJson));
		});
	...
	// StageEditor++: exact position steppers
	var posXStepper:PsychUINumericStepper;
	var posYStepper:PsychUINumericStepper;

	function syncCharJson(spr:StageEditorPlusMetaSprite)
	{
		...
		{
			case 'boyfriend':
				stageJson.boyfriend[0] = spr.x - char.positionArray[0];
				stageJson.boyfriend[1] = spr.y - char.positionArray[1];
			case 'gf':
				stageJson.girlfriend[0] = spr.x - char.positionArray[0];
				stageJson.girlfriend[1] = spr.y - char.positionArray[1];
			case 'dad':
				stageJson.opponent[0] = spr.x - char.positionArray[0];
				stageJson.opponent[1] = spr.y - char.positionArray[1];
		}
	}

	function updatePosTxt(spr:StageEditorPlusMetaSprite)
	{
		if(spr == null) { posTxt.visible = false; return; }
		var displayX:Float = Math.round(spr.x);
		var displayY:Float = Math.round(spr.y);
		var char:Character = cast spr.sprite;
		if(char != null)
		{
	...
	function addObjectTab()
	{
		...
		// StageEditor++: exact X/Y position inputs
		tab_group.add(new FlxText(objX, objY - 18, 150, 'Position (X/Y):'));
		var pxStepper:PsychUINumericStepper = new PsychUINumericStepper(objX, objY, 10, 0, 0, 100000, 0);
		var pyStepper:PsychUINumericStepper = new PsychUINumericStepper(objX + 70, objY, 10, 0, 0, 100000, 0);
		pxStepper.decimals = pyStepper.decimals = 0;
		pxStepper.onValueChange = function() {
			var selected = getSelected(false);
			if(selected != null)
			{
				selected.x = pxStepper.value;
				if(Std.isOfType(selected.sprite, Character))
					selected.x += cast(selected.sprite, Character).positionArray[0];
				syncCharJson(selected);
				unsavedProgress = true;
				updatePosTxt(selected);
			}
		};
		pyStepper.onValueChange = function() {
			var selected = getSelected(false);
			if(selected != null)
			{
				selected.y = pyStepper.value;
				if(Std.isOfType(selected.sprite, Character))
					selected.y += cast(selected.sprite, Character).positionArray[1];
				syncCharJson(selected);
				unsavedProgress = true;
				updatePosTxt(selected);
			}
		};
		tab_group.add(pxStepper);
		tab_group.add(pyStepper);
		posXStepper = pxStepper;
		posYStepper = pyStepper;
	...
	function updateSelectedUI()
	{
		...
		// StageEditor++: sync exact position steppers with selected sprite
		if(posXStepper != null && posYStepper != null)
		{
			var char2:Character = cast selected.sprite;
			posXStepper.value = char2 != null ? selected.x - char2.positionArray[0] : selected.x;
			posYStepper.value = char2 != null ? selected.y - char2.positionArray[1] : selected.y;
		}
	}
	...
	override function update(elapsed:Float)
	{
		...
		if(moveX != 0 || moveY != 0)
		{
			...
			if(spr != null)
			{
				...
				var newX:Float = spr.x + moveX;
				var newY:Float = spr.y + moveY;
				// StageEditor++: snap to grid
				if(snapToGrid)
				{
				...
				// StageEditor++: keep exact position steppers in sync while dragging
				if(posXStepper != null && posYStepper != null)
					updateSelectedUI();
			}
		}
		...
		if (FlxG.camera != null && FlxG.mouse.justPressed && !createPopup.visible && !FlxG.mouse.overlaps(UI_box, camHUD) && !FlxG.mouse.overlaps(UI_stagebox, camHUD) && !FlxG.mouse.overlaps(spriteList_box, camHUD))
		{
			...
			var mouseWorldY:Float = cam.scroll.y + (FlxG.mouse.screenY / cam.zoom);

			var maxLen:Int = spriteListRadioGroup.labels.length;
			for (j in 0...stageSprites.length)
			{
				var meta = stageSprites[j];
		...
		// StageEditor++: toggle grid (M) and snap (N)
		if(FlxG.keys.justPressed.M && !touchPad.buttonG.pressed)
		{
			showGrid = !showGrid;
			if(gridBackdrop != null) gridBackdrop.visible = showGrid;
			showOutput('Grid: ' + (showGrid ? 'ON' : 'OFF'));
		}
		if(FlxG.keys.justPressed.N && !touchPad.buttonG.pressed)
		{
			snapToGrid = !snapToGrid;
			showOutput('Snap to Grid: ' + (snapToGrid ? 'ON' : 'OFF'));
		}
	}
	...
	override function draw()
	{
		...
		{
			// StageEditor++: draw grid behind sprites
			if(gridBackdrop != null && gridBackdrop.visible)
				gridBackdrop.draw();
	...
	private function onLoadComplete(_):Void
	{
		...
		function loadSprite(imageToLoad:String)
		{
			if(_makeNewSprite != null)
			{
				...
				insertMeta(new StageEditorPlusMetaSprite({type: _makeNewSprite, name: findUnoccupiedName()}, new ModchartSprite()));
			}
			var selected = getSelected();
	...
	function tryLoadImage(spr:StageEditorPlusMetaSprite, imgPath:String)
	{
...
class StageEditorPlusMetaSprite
{
...
class StageEditorPlusAnimationSubstate extends MusicBeatSubstate {
	var bg:FlxSprite;
	...
	public var target:StageEditorPlusMetaSprite;
	
	var curAnim:Int = 0;
	var animsTxtGroup:FlxTypedGroup<FlxText>;

	var UI_animationbox:PsychUIBox;
	var camHUD:FlxCamera = cast(FlxG.state, StageEditorPlusState).camHUD;
	public function new()
	{
